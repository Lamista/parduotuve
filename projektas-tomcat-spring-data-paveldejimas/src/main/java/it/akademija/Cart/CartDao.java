...
public interface CartDao extends JpaRepository<Cart, Long> {
	...
	@Query("SELECT new it.akademija.Cart.GroupedCart(p.id as id, COUNT(p.id) as count) FROM Cart c join c.products p where c.id="
			+ "(SELECT c.id FROM Cart c WHERE c.username like :username) "
			+ "GROUP BY p.id")
	List<GroupedCart> getGroupedCartInfo(String username);
	
//	@Query("SELECT new it.akademija.Cart.GroupedCart(p as product, a.count as count) from Product p join "
